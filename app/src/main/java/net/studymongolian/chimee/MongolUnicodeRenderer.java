...
public class MongolUnicodeRenderer {
	...
	private String preFormatter(String mongolWord) {
		...
		for (int i = word.length() - 2; i > 0; i--) {
			if (word.charAt(i) == UNI_YA) {
				...
				if ((isVowel(nextChar) && nextChar != UNI_I) || !isVowel(prevChar)) {
					// insert FVS1
					word.insert(i + 1, FVS1);
				}
			}
		...
		for (int i = word.length() - 2; i > 0; i--) {
			if (word.charAt(i) == UNI_I) {
				...
				if (isMasculineVowel(word.charAt(i - 1)) && !isFVS(word.charAt(i + 1))) {
	...
	private void initIsolated() {
		...
		// TODO U Isol defualt should have a longer stem, so using initial.
		// change to GLYPH_ISOL_U if font glyph changes
		mIsolateMap.put("" + UNI_U, "" + GLYPH_INIT_U);
		...
		// TODO UE Isol defualt should have a long straight stem, so using initial.
		// change to GLYPH_ISOL_UE if font glyph changes
		mIsolateMap.put("" + UNI_UE, "" + GLYPH_INIT_UE);
	...
	private void initFinal() {
		...
		mFinalMap.put("" + UNI_GA + FVS3, "" + GLYPH_FINA_GA_FVS1);
	...
	private void initSuffixes() {
		...
		// U
		mSuffixMap.put("" + UNI_U, "" + GLYPH_ISOL_U_FVS1);
		// UE
		mSuffixMap.put("" + UNI_UE, "" + GLYPH_ISOL_UE_FVS2);
	...
	private static final char GLYPH_ISOL_U = '\uE3A4';
